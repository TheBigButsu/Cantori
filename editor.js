...
(function () {
  ...
  function jsonHint(coll) {
    return ({
      ...
      loot: "Rarity table, stat pool, and tier-by-floor bands. dropWeights = the gold/gear/consumable split of a floor's random drops (favour gear so weapons aren't drowned out). The split alone cannot raise two categories at once \u2014 the third pays for it \u2014 so the per-floor COUNT lives in game.js beside spawnItems, and the two were tuned together against measured floors: gear \u00d71.15 and consumables \u00d71.25 with gold held flat. Tune by measuring, not by arithmetic: run-to-run variation is around \u00b14% even over 700 generated floors. categoryWeights = odds of each gear slot (no trinket — trinkets are boss-only). trinketRarity = the blue/purple/gold floor for boss trinkets. identifyXp = how much EXPERIENCE it costs to learn an unidentified piece \u2014 one flat number for every drop, no rarity or depth scaling, because two rings filling at different speeds for invisible reasons made the progress bar meaningless. Every point of XP the player earns advances every unidentified thing they are wearing by one. For scale, a full clear yields about 7 XP at depth 1, 11 at depth 3, 21 at depth 6, 31 at depth 9 and 60 at depth 12, plus 75/175/375 on the boss floors at 5/10/15 \u2014 so 20 is roughly a floor early on and rather less than one deep in. (Enchants have their own tab.)",
