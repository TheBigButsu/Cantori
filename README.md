...
Open the game once with a connection and it keeps itself — page, scripts, sprites
and all. After that it runs with the network off, in the browser or launched from
the home screen, and nothing about the run changes.

A badge tells you where you stand, because "it'll probably work offline" is worth
nothing at the gate:

| Badge | Means |
|---|---|
| `✈ Saving for offline… 41%` | still downloading — stay on the page |
| `✈ Ready to play offline` | every file is saved; safe to fly |
| `✈ Not fully saved` | something didn't download — reload with a signal |
| `✈ Playing offline` | it's running from the saved copy right now |

For the best of it, **add it to your home screen** first (iOS: Share → Add to Home
...
sw.js                 service worker — keeps a copy of the site for offline play
offline.js            registers the worker and shows the "ready to fly" badge
